...
**App flow is a three-state machine** in `App.jsx` (`appState`): `'loading'` → `'auth'` → `'app'`, rendering `LoadingScreen`, `AuthScreen`, or the main UI. On `LoadingScreen` finish, `handleLoadingDone` checks `supabase.auth.getSession()` and skips straight to `'app'` if a session exists. Within the app, `view` (`'home'` | `'community'` | `'leaderboard'` | `'you'`) selects `DispatchHome` (the map-first home, replacing the old card-dashboard `HomeFeed`: a full-bleed forest-ink Leaflet map is the screen itself, with floating ink chrome — wordmark bar and a "Got trash?" compose pill — on top, and a draggable [ui/BottomSheet.jsx](src/components/ui/BottomSheet.jsx) holding the Go Online toggle, nearby open jobs as slim `JobRow` items, and the viewer's own active pickups), `FeedView`, `LeaderboardView`, or `ProfileView`; `BottomNav` switches views, and its raised center button opens the `Conversations` inbox → `MessageThread` overlay (which hosts per-request chat, the pickup journey rail, after-photo review, and payment/rating actions). Auth is real — `AuthScreen` uses `supabase.auth.signInWithPassword` / `signUp` (sign-up stores `name` in user metadata and may require email confirmation). An idle-logout hook signs users out after 10 minutes of inactivity. `ProfileView` hosts three settings sheets: [AccountSheet.jsx](src/components/AccountSheet.jsx) (avatar upload to the `avatars` bucket, display name, email/password via `supabase.auth.updateUser`), [NotificationsSheet.jsx](src/components/NotificationsSheet.jsx) (toggles persisted in `profiles.notification_prefs`), and [GuidelinesSheet.jsx](src/components/GuidelinesSheet.jsx) (static community guidelines); its former theme-picker row is gone now that the app ships a single `eco-premium` theme (the switcher mechanism in `themes.js` stays wired for a future variant). The prefs gate in-app alert toasts fired from `App.jsx`'s `app-alerts` realtime channel: request status transitions moved by the other party, new messages (RLS-scoped table-wide subscription), and new community posts.
...
- [useOffers](src/hooks/useOffers.js) — pending `price_offers` for one request (poster side); fetches once, tracks INSERTs (enriched with the collector's profile) and drops offers that leave `pending`. Feeds the counter-offer cards in `DispatchRadar`.
...
- `collector_presence` — online collectors: `collector_id` (PK), `lat`, `lng`, `online` (boolean), `updated_at`.
- `price_offers` — counter-offers: `id`, `request_id`, `collector_id`, `price`, `status` (`pending|accepted|declined`), `created_at`.
- `push_subscriptions` — web push device endpoints: `id`, `user_id`, `endpoint`, `p256dh`, `auth`, `created_at`.

**Presence subsystem:** Collectors explicitly toggle their availability in `DispatchHome`'s Go Online row. While online with the app open, the client sends location heartbeats to update the `collector_presence` table. Only collectors marked as online and within range receive dispatch pings.

**Web Push subsystem:** The `push-dispatch` Supabase Edge Function is triggered via a DB webhook on `requests` INSERT. It calculates distances, finds online collectors within 5 km using `collector_presence` and `push_subscriptions`, and sends system notifications using standard VAPID keys (public key via `VITE_VAPID_PUBLIC_KEY` in `.env`, private key via Edge Function secrets). `src/sw.js` handles the `push` events to display the notification, and `notificationclick` to focus the app.
...
**Every color goes through a design token, and the app ships a single theme.** `src/index.css` defines the full token set (`--surface`, `--surface-card`, `--text-primary/-secondary/-muted`, `--border`, `--brand`, `--on-brand`, `--brand-ink`, `--accent`, `--success/--warning/--danger`, `--radius-*`, `--shadow-*`, per-tag chip vars, plus the ink-chrome set below) with the `eco-premium` defaults for first paint; [src/lib/themes.js](src/lib/themes.js) exports only `eco-premium` and rewrites the tokens on `<html>` — the switcher mechanism (and `data-dark` attribute) is kept wired for a future night variant, but ProfileView's theme-picker UI is currently removed. `eco-premium` is cream-surfaced (`--surface`/`--surface-card`) with a separate **forest-ink map-chrome set** — `--surface-ink`, `--border-ink`, `--text-on-ink`/`--text-on-ink-muted` — used by `DispatchHome`'s floating chrome, `BottomNav`, and other map-adjacent surfaces. `--accent` (gold) is reserved for money and "live" signals: prices, the online-toggle knob, `CountdownRing`, radar pins, and `BottomNav`'s center button; `--live-dot` (mint) marks online collectors on the map. Leaflet tiles are always rendered forest-ink via an unconditional `.leaflet-layer` CSS filter in `index.css` — there is no more `data-dark`-gated tile rule. Fraunces (via `--font-display`) sets screen titles, section headers, and every peso amount; DM Sans remains body/UI. **Never hardcode hex in components** (exceptions: overlays on photos and Leaflet popup internals, which are theme-independent).

Shared primitives live in [src/components/ui/](src/components/ui/) — `Button`, `Card`, `Chip`, `Input`/`TextArea`, `Sheet` (the app's single modal idiom), `BottomSheet` (draggable three-detent sheet — peek/half/full — used by `DispatchHome`), `EmptyState`, `Avatar`. Interactive states that inline styles can't express (`:focus` rings, press feedback, sheet/skeleton/pop animations, radar-ring pulse, dot glow) are the `.tt-*` classes in `index.css`, all reduced-motion aware.

**Visual verification without a backend:** the dev server serves `/preview.html` ([src/dev/preview.jsx](src/dev/preview.jsx)), which renders shell/screens with fixture data — `?screen=shell|cards|sheet|feed|profile|board|inbox|pay|confirmpay|thread|credential|settings|notifprefs|guidelines|dispatchhome|radar|offer|bottomsheet|dispatchmap&theme=eco-premium` (the `theme` param currently only resolves to `eco-premium`). It is not part of the production build. Maps use Leaflet / react-leaflet with imported marker assets.
